-- Fix security issues and complete category setup

-- Update categories to include sort_order where missing
UPDATE categories 
SET sort_order = COALESCE(sort_order, 0) 
WHERE sort_order IS NULL;

-- Add remaining subcategories if they don't exist (avoiding conflicts)
INSERT INTO categories (name, description, type, parent_id, slug, sort_order) 
SELECT v.name, v.description, v.type::listing_category_type, v.parent_id::uuid, v.slug, v.sort_order
FROM (VALUES
