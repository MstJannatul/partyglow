-- Update simple_booking_availability function to accept optional exclude_booking_id parameter
...
-- Update validate_simple_booking trigger function to exclude current booking during updates
...
AS $function$
BEGIN
  -- For UPDATE operations, exclude the current booking from conflict checks
  -- For INSERT operations, check all bookings for conflicts
  IF TG_OP = 'UPDATE' THEN
    IF NOT simple_booking_availability(NEW.vendor_id, NEW.start_date, NEW.end_date, NEW.id) THEN
