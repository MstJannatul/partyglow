...
SELECT v.id::uuid, v.name, v.description, v.type::listing_category_type, v.slug, v.sort_order::integer FROM (VALUES
...
SELECT v.name, v.description, v.type::listing_category_type, v.parent_id::uuid, v.slug, v.sort_order::integer
FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE categories.name = v.name AND categories.parent_id = v.parent_id::uuid);
